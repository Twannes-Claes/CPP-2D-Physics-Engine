...
public:
	...
	explicit Polygon(std::vector<SDL_FPoint> vertices, float boundingRadius);
	...
	float GetMomentOfInteria(float) const override;

	const std::vector<SDL_FPoint>& GetVertices() const { return m_TransformedPoints; }

	virtual glm::vec2 GetEdge(uint32_t index) const;
